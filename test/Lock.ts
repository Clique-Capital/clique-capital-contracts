...
describe("Clique Capital Payouts", () => {
  let cliqueCapitalPayout: Awaited<
    ReturnType<typeof hre.viem.deployContract<"CliqueCapitalPayout">>
  >;

  before(async () => {
    cliqueCapitalPayout = await hre.viem.deployContract("CliqueCapitalPayout");
    console.log("CliqueCapitalPayout address:", cliqueCapitalPayout.address);
  });

  describe("Test Deployment", () => {
    it("Clique Capital Payouts Deployed", async () => {
      expect(cliqueCapitalPayout.address).to.not.be.undefined;
    });
  });
});
